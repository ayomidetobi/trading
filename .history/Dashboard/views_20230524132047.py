...
class IndexView(LoginRequiredMixin, TemplateView):
    ...
    def generate_random_trades_periodically(self, minutes):
        while True:
            ...
            self.get_profit_loss_data().append(float(profit_loss))
            time.sleep(minutes * 60)

    def start_random_trades_thread(self, minutes):
        t = threading.Thread(target=self.generate_random_trades_periodically, args=(minutes,))
        t.start()
...
class UpdateChartDataView(View):
    def get(self, request, *args, **kwargs):
        ...
        minutes = 1.0  # Change this value to adjust the interval between trades
        response.context_data['random_trades_thread'] = IndexView..start_random_trades_thread(minutes)
        profit_loss_data = response.context_data['profit_loss_data']
