...
class Trade(models.Model):
    ...
    def generate_random_trade(cls):
        profit_loss = round(random.uniform(-100, 100), 2)
        trade = cls(profit_loss=profit_loss)
        trade.save()
        return trade
    
    def generate_random_trades_periodically(cls, minutes, num_trades):
        trades = []
        for _ in range(num_trades):
            trade = cls.generate_random_trade()
            trades.append(trade.profit_loss)
            timezone.sleep(minutes * 60)
        return trades
